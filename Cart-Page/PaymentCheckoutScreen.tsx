...
const MOCK_PAYMENT_METHODS: PaymentMethod[] = [
  ...
  },
  {
    ...
    type: 'upi',
    label: 'Google Pay',
    subLabel: 'alex.johnson@okicici',
    icon: '🔗',
    upiId: 'alex.johnson@okicici',
  },
  {
    id: 'pm4',
    type: 'upi',
  ...
  return (
    <TouchableOpacity onPress={onToggle} activeOpacity={0.85}>
      <View style={toggleStyles.row}>
        ...
        <Animated.View style={[toggleStyles.track, { backgroundColor }]}>
...
// ─── Gold Radio Button ──────────────────────────────────────────────────────
interface GoldRadioProps {
  selected: boolean;
}

const GoldRadio: React.FC<GoldRadioProps> = ({ selected }) => {
  ...
  return (
    <View style={radioStyles.outer}>
      <Animated.View style={[radioStyles.inner, { transform: [{ scale: scaleAnim }] }]} />
    </View>
  );
};
...
const PaymentSuccessModal: React.FC<PaymentSuccessModalProps> = ({
  ...
  const checkRotateStyle = checkRotate.interpolate({
    inputRange: [0, 1],
      ...
      <Animated.View style={[modalStyles.overlay, { opacity: overlayOpacity }]}>
        ...
        >
          ...
          <View style={modalStyles.checkContainer}>
            ...
            />
            {/* Glow ring */}
            <Animated.View style={[modalStyles.glowRing, { transform: [{ scale: glowPulse }] }]} />
            {/* Check circle */}
            <Animated.View
              style={[
                modalStyles.checkCircle,
                {
                  transform: [
                    { scale: checkScale },
                    { rotate: checkRotateStyle },
                  ],
                },
              ]}
            >
          ...
          {/* Content */}
          <Animated.View style={[modalStyles.content, { opacity: contentOpacity }]}>
            <Text style={modalStyles.successTitle}>Payment Successful!</Text>
            <Text style={modalStyles.successSubtitle}>
              Your order has been confirmed and is being prepared 🍽️
            </Text>

            {/* Order Info Card */}
            <View style={modalStyles.orderInfoCard}>
              <View style={modalStyles.orderInfoRow}>
                <View style={modalStyles.orderInfoItem}>
                  <Text style={modalStyles.orderInfoLabel}>Order ID</Text>
                  <Text style={modalStyles.orderInfoValue}>{orderId}</Text>
                </View>
                <View style={modalStyles.orderInfoDivider} />
                <View style={modalStyles.orderInfoItem}>
                  <Text style={modalStyles.orderInfoLabel}>Est. Delivery</Text>
                  <Text style={modalStyles.orderInfoValue}>{estimatedTime}</Text>
                </View>
              </View>
            </View>
            ...
            <View style={modalStyles.statusRow}>
              {['Confirmed', 'Preparing', 'On the Way', 'Delivered'].map((step, i) => (
                <View key={step} style={modalStyles.statusItem}>
                  <View style={[modalStyles.statusDot, i === 0 && modalStyles.statusDotActive]}>
                    {i === 0 && <Text style={modalStyles.statusDotText}>✓</Text>}
                  </View>
                  {i < 3 && <View style={[modalStyles.statusLine, i === 0 && modalStyles.statusLineActive]} />}
                  <Text style={[modalStyles.statusLabel, i === 0 && modalStyles.statusLabelActive]}>
                    {step}
                  </Text>
                </View>
              ))}
            ...
            {/* CTA Buttons */}
            <AnimatedPressable onPress={onTrackOrder} style={modalStyles.trackBtn}>
              <Text style={modalStyles.trackBtnText}>📍 Track My Order</Text>
            </AnimatedPressable>

            <TouchableOpacity onPress={onTrackOrder} style={modalStyles.backHomeBtn}>
              <Text style={modalStyles.backHomeBtnText}>Back to Home</Text>
            </TouchableOpacity>
          </Animated.View>
        </Animated.View>
      </Animated.View>
...
const ProcessingOverlay: React.FC<ProcessingOverlayProps> = ({ visible }) => {
      ...
      const createDotAnim = (anim: Animated.Value, delay: number) =>
        Animated.loop(
          Animated.sequence([
            ...
            Animated.spring(anim, { toValue: -10, useNativeDriver: true, speed: 20, bounciness: 10 }),
            Animated.spring(anim, { toValue: 0, useNativeDriver: true, speed: 20, bounciness: 10 }),
            Animated.delay(400),
          ])
        );
      dotAnims.forEach((anim, i) => createDotAnim(anim, i * 150).start());
    } else {
  ...
  return (
    <Animated.View
      ...
      style={[processingStyles.overlay, { opacity: fadeAnim }]}
    >
      <View style={processingStyles.card}>
        <View style={processingStyles.dotsRow}>
          {dotAnims.map((anim, i) => (
            <Animated.View
              key={i}
              style={[processingStyles.dot, { transform: [{ translateY: anim }] }]}
            />
          ))}
        </View>
        <Text style={processingStyles.text}>Processing Payment…</Text>
        <Text style={processingStyles.subText}>Please don't close this screen</Text>
      </View>
    </Animated.View>
  );
};
...
const PaymentCheckoutScreen: React.FC<PaymentCheckoutScreenProps> = ({
  grandTotal = 42.76,
  onBack,
}) => {
  ...
  const handleAddressToggle = () => {
    ...
    Animated.spring(addressExpandAnim, {
      toValue,
  ...
  const handlePay = () => {
    ...
    Animated.sequence([
      Animated.spring(buttonScaleAnim, { toValue: 0.94, useNativeDriver: true, speed: 50 }),
      Animated.spring(buttonScaleAnim, { toValue: 1, useNativeDriver: true, speed: 30, bounciness: 8 }),
    ]).start();
  ...
  const addressMaxHeight = addressExpandAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 220],
  });

  const arrowRotation = addressExpandAnim.interpolate({
    inputRange: [0, 1],
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
          ...
          <TouchableOpacity onPress={handleAddressToggle} activeOpacity={0.8} style={styles.addressHeader}>
            <View style={styles.addressHeaderLeft}>
              <View style={styles.goldPin}>
                <Text style={styles.goldPinIcon}>📍</Text>
              </View>
              <View style={styles.addressHeaderText}>
                ...
                <Text style={styles.addressHeaderStreet} numberOfLines={1}>
                  {selectedAddr.street}
                </Text>
              </View>
            </View>
            <Animated.Text style={[styles.addressChevron, { transform: [{ rotate: arrowRotation }] }]}>
              ▼
            </Animated.Text>
          </TouchableOpacity>

          {/* Expandable Address List */}
          <Animated.View style={[styles.addressExpand, { maxHeight: addressMaxHeight, overflow: 'hidden' }]}>
            ...
            {MOCK_ADDRESSES.map(addr => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  selectedAddress === addr.id && styles.addressOptionSelected,
                ]}
              >
              ...
              {group.methods.map((method, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    selectedPayment === method.id && styles.paymentMethodRowSelected,
                    index < group.methods.length - 1 && { marginBottom: 8 },
                  ]}
                >
                  ...
                  <View style={styles.paymentMethodText}>
                    ...
                    {method.subLabel && (
                      <Text style={styles.paymentMethodSub}>{method.subLabel}</Text>
                    )}
                  </View>
        ...
        <View style={styles.section}>
          ...
          <GoldToggle
            value={saveCard}
            onToggle={() => setSaveCard(v => !v)}
            label="Save card for future payments"
            subLabel="Your data is encrypted and secure"
          />
        </View>
...
const styles = StyleSheet.create({
  ...
  addressHeaderLeft: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  goldPin: {
    width: 44,
    height: 44,
    borderRadius: 12,
